    ...
    Column(
        ...
        Box(modifier = Modifier
            ...
            if (isListCurrentlyRefreshing && isBookListEmpty) {
                // Состояние 1: Начальная загрузка (список пуст, идет обновление)
                Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
                    CircularProgressIndicator()
                }
            } else if (isBookListEmpty) { // Сюда попадаем, если !isListCurrentlyRefreshing && isBookListEmpty
                // Состояние 2: Список пуст (и не обновляется)
                Box(modifier = Modifier.fillMaxSize().padding(16.dp), contentAlignment = Alignment.Center) {
                    Text("Книги не найдены")
                }
            } else { // Сюда попадаем, если !isBookListEmpty (список не пуст)
                // Состояние 3: Список не пуст (может обновляться в фоне, и тогда сработает PullRefreshIndicator)
                LazyColumn(
                    modifier = Modifier.fillMaxSize(), 
